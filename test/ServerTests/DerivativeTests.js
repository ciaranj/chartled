  ...
  describe('derivative', function(){
    ...
    it('should calculate the difference between consecutive metric values in the given series list (multiple)', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            ...
                            assert.deepEqual( [null,10,-9,null,null,null,null,null,-9,null,null,10], result.seriesList[0].data.values );
                            assert.deepEqual( [null,null,null,null], result.seriesList[1].data.values );
                            assert.deepEqual( [null,2,null,null,null,null], result.seriesList[2].data.values );
                            assert.deepEqual( [null,3,null,null,null,2], result.seriesList[3].data.values );
                            assert.deepEqual( [null,null,3,null,null,1,null], result.seriesList[4].data.values );
                            assert.deepEqual( [null,null,4,null,null,1,null], result.seriesList[5].data.values );
                            assert.deepEqual( [null,null,null,null,null,null,null], result.seriesList[6].data.values );
                            assert.deepEqual( [null,1,1,1,1], result.seriesList[7].data.values );
                            done();
                    })
                    .end();
    });
    it('should sum over time the metric values in the given series list (multiple)', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            ...
                            assert.equal( 12, result.seriesList[0].data.values.length );
                            assert.deepEqual( [null,118,122,-233,-9,196,-195,10,-9,10,-9,2994], result.seriesList[0].data.values );
                            done();
                    })
                    .end();
    });
    it('should update the metric name correctly for multiple series', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            assert.equal( 8, result.seriesList.length );
                            assert.equal( "derivative(foo.bar)", result.seriesList[0].name );
                            assert.equal( "derivative(foo.car)", result.seriesList[1].name );
                            assert.equal( "derivative(foo.far)", result.seriesList[2].name );
                            assert.equal( "derivative(foo.gar)", result.seriesList[3].name );
                            assert.equal( "derivative(foo.rar)", result.seriesList[4].name );
                            assert.equal( "derivative(foo.tar)", result.seriesList[5].name );
                            assert.equal( "derivative(foo.zar)", result.seriesList[6].name );
                            assert.equal( "derivative(foo.war)", result.seriesList[7].name );
                            done();
                    })
                    .end();
    });
    ...
    it('should update the metric name correctly for ranges', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            ...
                            assert.equal( "derivative(foo.1ar)", result.seriesList[0].name );
                            assert.equal( "derivative(foo.2ar)", result.seriesList[1].name );
                            assert.equal( "derivative(foo.12ar)", result.seriesList[2].name );
                            done();
                    })
                    .end();
    });
