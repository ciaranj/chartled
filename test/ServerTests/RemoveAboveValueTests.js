  ...
  describe('removeAboveValue', function(){
    ...
    it('should update the metric name correctly for multiple series', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            assert.equal( 2, result.seriesList.length );
                            assert.equal( "removeAboveValue(foo.bar,2)", result.seriesList[0].name );
                            assert.equal( "removeAboveValue(foo.tar,2)", result.seriesList[1].name );
                            done();
                    })
                    .end();
    });
    it('should update the metric name correctly for wildcards', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            assert.equal( 2, result.seriesList.length );
                            assert.equal( "removeAboveValue(poo.bar,2)", result.seriesList[0].name );
                            assert.equal( "removeAboveValue(foo.tar,2)", result.seriesList[1].name );
                            done();
                    })
                    .end();
    ...
    it('should update the metric name correctly for ranges', function(done) {
        ...
        TargetParser.parse( metric )(ctx)
                    .then(function (result) {
                            assert.equal( 3, result.seriesList.length );
                            assert.equal( "removeAboveValue(foo.1ar,2)", result.seriesList[0].name );
                            assert.equal( "removeAboveValue(foo.2ar,2)", result.seriesList[1].name );
                            assert.equal( "removeAboveValue(foo.12ar,2)", result.seriesList[2].name );
                            done();
                    })
                    .end();
    });
