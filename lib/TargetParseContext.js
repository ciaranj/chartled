            ...
            .then( function( populatedMetric ) {
                for( var k in populatedMetric.seriesList) {
                    ...
                    for( var i in series.data.values ) {
                      series.data.values[i] *= factor;
                    }
                    series.name= "scale("+ series.name+","+ factor +")";
                }
                populatedMetric.name= "scale("+ populatedMetric.name +","+factor+")";
                deferred.resolve( populatedMetric );
            })
            .fail( function( err ) { 
                deferred.reject( err );
...
TargetParseContext.prototype.bestFit= function( pa ) {
  ...
  return Q.when( pa, function(value) {
      ...
      that.$chkSeries( value, true).then( function( populatedMetric ) {
        ...
        fittedSeries.info.aggregationMethod= "average";
...
for (k = 0; k < n; k++) {
   ...
   toty = toty + y[k];
   totz = totz + z[k];
   toty2 = toty2 + pow(y[k], 2.0);
   totyz = totyz + (y[k] * y[k]);
 
}
 
*a1 = (totyz - ((toty * totz)/(double)n)) / (toty2-(pow(toty,2.0)/(double)n));
*a0 = (totz - ((*a1) * toty)) / (double)n;
