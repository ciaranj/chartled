        ...
        if( metric.expanded ) {
            ...
            that.metricsStore.getAvailableMetrics( function( err, availableMetrics ) {
                ...
                for( var availableMetric in availableMetrics ) {
                    if( metric.matches( availableMetric ) ) {
...
TargetParseContext.prototype._populateMetrics= function( pMetric, from, to ) {
    var that= this;
    return Q.when( pMetric, function( metric ) {
        var deferred = Q.defer();
        if( metric.expanded && metric.seriesList.length == 0 ) {
            ...
            for( var i=0;i< metric.seriesList.length;i++) {
                (function(j) {
                    that.metricsStore.fetchMetricData( metric.seriesList[j].realName, from, to, function( err, data ) {
                        resultDownCount--;
                        if( err ) {
                             errored=true;
                             deferred.reject(err);
...
TargetParseContext.prototype.$expandAndPopulateMetricOrMetrics= function(metricsToPopulate, offset) {
  var deferred = Q.defer();
  var that= this;
  if( metricsToPopulate.length == 0 ) {
    deferred.resolve([]);
  }
  else {
    var populatedMetrics= [];
    function checkNextMetric() {
      if( metricsToPopulate.length == 0 ) {
        deferred.resolve( populatedMetrics );
      } else {
        that.$chkSeries( metricsToPopulate.shift(), true, offset).then( function( populatedMetric ) {
          populatedMetrics.push(populatedMetric);
          setImmediate(checkNextMetric);
        });
      }
    }
    checkNextMetric();
...
function joinPopulatedMetricNames( populatedMetrics ) {
  var names= [];
  for(var i=0;i<populatedMetrics.length;i++) {
    names.push( populatedMetrics[i].name );
  }
  return names.join()
}
...
TargetParseContext.prototype.avg=TargetParseContext.prototype.averageSeries= function () {
    ...
    return Q.spread( arguments, function() {
        var deferred = Q.defer();
        that.$expandAndPopulateMetricOrMetrics( Array.prototype.slice.call(arguments, 0) ).then( function( populatedMetrics ) {
        var result= new Matcher("avg(" + joinPopulatedMetricNames(populatedMetrics) +")");
        result.expanded= true;
        result.populated= true;

        // Potentially the first found series is on a metric other than the first provided one. :/
        var firstSeries= null;
        for(var i=0;i<populatedMetrics.length;i++) {
          if( populatedMetrics[i].seriesList.length >0 ) {
            firstSeries= populatedMetrics[i].seriesList[0];
            break;
          }
        }
        if( firstSeries ) {
          ...
          result.seriesList[0].data.tInfo= firstSeries.data.tInfo;
          result.seriesList[0].info.aggregationMethod=firstSeries.info.aggregationMethod;
          result.seriesList[0].name= "avg(" + joinPopulatedMetricNames(populatedMetrics) +")";
        
          var seriesValues= [];
          for(var i=0;i<populatedMetrics.length;i++) {
            for(var j=0;j<populatedMetrics[i].seriesList.length;j++) {
              seriesValues.push( populatedMetrics[i].seriesList[j].data.values );
            }
          }
          
          var seriesLen= seriesValues[0].length;
          var seriesCount= seriesValues.length;
          var values= [];
          for( var i=0;i<seriesLen;i++) {
            var divisor= 0;
            for( var k= 0; k< seriesCount;k++) {
              if( !isNone(seriesValues[k][i]) ) {
                if( typeof(values[i]) == 'undefined' ) values[i] = seriesValues[k][i];
                else values[i] += seriesValues[k][i];
                divisor++;
              }
            }
            if( typeof(values[i]) != 'undefined' && divisor > 1 ) values[i] /= divisor;
          }
          result.seriesList[0].data.values=  values;
        }
  ...
  return Q.spread( [pa, pintervalString], function(value, intervalString) {
    ...
    that.$chkSeries( value, true, delta, delta )
        .then( function( populatedMetric ) {
            for( var seriesKey in populatedMetric.seriesList ) {
                var series= populatedMetric.seriesList[seriesKey];
                series.name= "timeShift(" +  populatedMetric.name + ",\""+ originalIntervalString +"\")";
