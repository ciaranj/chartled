...
// If no timezone is provided, the code will assume Europe/London for any passed (non utc-based) absolute times.
module.exports.parseATTime= function(momentStr, tz) {
  if( momentStr == null || typeof(momentStr) == "undefined") throw new Error("No time string available to parse");
  if( tz == null || typeof(tz) == "undefined") tz= "Europe/London";
  momentStr= momentStr.trim().toLowerCase().replace(/_/g,'').replace(/,/g,'').replace(/ /g,'');
  var result= ATTimeParser.parse(momentStr, {"tz": tz});
  return result.unix();
}
...
module.exports.parseTimeOffset= function(offset) {
  ...
  var result= ATTimeParser.parse(offset, {startRule: "offset"});
